...
export default function AsistentesBackupPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const verifyAndFetch = async (eid?: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/asistentes?eventId=${encodeURIComponent(id)}`);
      if (!res.ok) {
  ...
  const handleCheckin = async (ticketId: string, currentStatus: boolean) => {
    ...
    try {
      const res = await fetch('/api/admin/asistentes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticketId, action, eventId }),
      });
